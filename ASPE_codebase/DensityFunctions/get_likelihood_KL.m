function [maxent_lik,lik_der] = get_likelihood_KL(params,scores,labels,lambda,distHandler0,disthandler1,m0,m1)
...
% value of modified likelihood
	...
	maxent_lik =  sum(log(marg_unlab)) + sum(log(p1*pos_lik1))  + sum(log(p0*neg_lik0)) + ...
...
% derivative of likelihood on the parameters
	...
	lik_der(1) = sum( (unlabeled_lik1 - unlabeled_lik0)./marg_unlab ) + (pos/p1 - neg/p0) + ...
	...
	lik_der(2:m0+1) = sum( p0 * log_der_unlab0 )' + sum( bsxfun(@times,1./neg_lik0,neg_lik_der0) )' + ...
		lambda*sum( bsxfun(@times , -(1+log(unlab_lik1)).*unlab_lik1 , log_der_unlab0*p0) )' + ...
		lambda*sum( bsxfun(@times , p0*(1+log(unlab_lik0)).*(1- unlab_lik0) , log_der_unlab0) )';

	lik_der(m0+2:m1+m0+1) = sum( p1 * log_der_unlab1 )' + sum( bsxfun(@times,1./pos_lik1,pos_lik_der1) )' + ...
		lambda*sum( bsxfun(@times, -(1+log(unlab_lik0)).*unlab_lik0 , log_der_unlab1*p1) )' + ...
