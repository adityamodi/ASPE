function [maxent_lik,lik_der] = get_likelihood_KL_weighted(params,scores,labels,lambda,distHandler0,disthandler1,m0,m1)
...
% derivative of likelihood on the parameters
	...
	lik_der(1) = sum( (unlabeled_lik1 - unlabeled_lik0)./marg_unlab )/unlab + (pos/p1 - neg/p0)/lab + ...
		lambda*sum( bsxfun(@times,log(unlab_lik1) + 1, unlab_lik1/p1 - unlab_lik1 .* ((unlabeled_lik1 - unlabeled_lik0)./marg_unlab) ) )/unlab + ...
		lambda*sum( bsxfun(@times,log(unlab_lik0) + 1, -unlab_lik0/p0 - unlab_lik0 .* ((unlabeled_lik1 - unlabeled_lik0)./marg_unlab) ))/unlab;

	lik_der(2:m0+1) = sum( p0 * log_der_unlab0 )'/unlab + sum( bsxfun(@times,1./neg_lik0,neg_lik_der0) )'/lab + ...
	...
	lik_der(m0+2:m1+m0+1) = sum( p1 * log_der_unlab1 )'/unlab + sum( bsxfun(@times,1./pos_lik1,pos_lik_der1) )'/lab + ...
